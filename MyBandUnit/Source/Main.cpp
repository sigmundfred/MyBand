...
class Application    : public juce::JUCEApplication, juce::InterprocessConnectionServer
{
public:
    //==============================================================================
    Application() = default;
    ...
    {
        this->beginWaitingForSocket(9905);
        startZeroconf();
        mainWindow.reset (new MainWindow ("MyBandUnit", &coreUnit, *this));
    }
    ...
    void startZeroconf()
    {
        ...
        zeroConfManager = std::unique_ptr<ZeroConfManager>(new ZeroConfManager(&monitor, &registrar));
        service = std::unique_ptr<ZeroConfService>(new ZeroConfService());
        
        service->setPort((juce::uint16)(9905));
        service->setServiceName("Myband unit");
        service->setRegType("_mybandunit._udp");
        
        zeroConfManager->registerService(service.get());
    }
    ...
    {
        juce::Logger::writeToLog("GUI connection");
        return &coreUnit;
    }
